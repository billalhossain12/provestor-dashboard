...
export default function UploadProfileImage() {
	...
	return (
		<div onClick={() => document?.getElementById('propertyImage')?.click()}>
			<div className="md:w-[180px] w-full">
				<div className="h-[150px] flex flex-col">
					<div className="flex-1">
						{selectedFile.length > 0 ? (
							<img
								className="md:h-full h-[100px]"
								src={`${URL.createObjectURL(selectedFile?.[0])}`}
								alt=""
							/>
						) : (
							<div className="flex justify-center items-center h-full text-[3rem] text-primary cursor-pointer">
